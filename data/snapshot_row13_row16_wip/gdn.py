...
def naive_gated_delta_rule(
    ...
    with _fp32_guard(query.device.type):
        ...
        for t in range(seq_len):
            ...
            kv_mem = (state * key[:, :, t].unsqueeze(-1)).sum(dim=-2)
            delta = (value[:, :, t] - kv_mem) * beta[:, :, t].unsqueeze(-1)
            state = state + key[:, :, t].unsqueeze(-1) * delta.unsqueeze(-2)
...
class GatedDeltaNet(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor,
        ...
        with _fp32_guard(hidden_states.device.type):
            ...
            out = self.o_norm(core_attn_out, z.transpose(1, 2))
