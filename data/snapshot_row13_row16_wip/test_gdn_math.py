...
def main() -> None:
    ...
    g10 = -(0.01 + torch.rand(2, 3, 64, requires_grad=True))
    beta10 = torch.sigmoid(torch.randn(2, 3, 64, requires_grad=True))
    o10, _ = chunk_gated_delta_rule(q10, k10, v10, g10, beta10, chunk_size=32)
    loss = (o10 * torch.randn_like(o10)).pow(2).mean()
    loss.backward()
    names = ["q", "k", "v", "g", "beta"]
    grads = [q10.grad, k10.grad, v10.grad, g10.grad, beta10.grad]
