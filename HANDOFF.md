...
- **Status: Tasks 0-5 + GPU gates DONE; Task 6 arms RUNNING (control DONE, fill running; then gate/drop/hybrid strictly sequential); Task 7 honesty gates live.** Everything in this section is THIS track's work; shared-file edits were targeted only — other agents' uncommitted edits in these same files are intentionally NOT included in this track's commits.
...
- Task 6.1 so far: control arm (scripts/train.py, configs/mount_control.yaml) clean, zero interruptions, best eval 1.8094 @1000 (curve 2.22@500 -> 1.81@1000). Arms strictly sequential single-GPU: fill (running) -> gate -> drop -> hybrid, each ~1000 steps at ~8.5-10 s/it (frozen-teacher forward per micro-batch). Honesty gates (plan Task 7): eval beyond +20% of control at matched steps = cooperative stop of THAT arm only; wall-clock is the headline; failures reported FAIL/SKIPPED with reasons.
