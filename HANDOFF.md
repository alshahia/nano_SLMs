...
- venv: `.venv` (uv-managed, CPython 3.12.9). NEVER pip; use
  ...
  uv pip install --python .venv transformers==5.16.1 datasets==5.0.1 accelerate==1.14.0 bitsandbytes==0.50.2 tensorboard==2.21.0 pyyaml
  ```
...
2. (Optional, user-approved) M3: rerun vram_probe with --seq 1024 to check ctx
   headroom **on the new 6 GB GPU** (M1's 4.24 GB was measured on the 8 GB
   RTX 4000), then prepare target data (more rows) and launch target run.
