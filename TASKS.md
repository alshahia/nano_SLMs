...
|---|---|---|---|---|
| 1 | M3 target run (226.5M, ctx 1024, 5000 steps) — monitor to completion | `in_progress` | After ANY interruption re-run the exact train command (auto-resume, zero flags). Fresh relaunch 2026-09-06 10:56 on MUO4QK5; sustained ~6.5–7.9 s/it → ETA ~8–9 h | commits 5ab9096 (step-500 PASSED, eval 2.8567) + step-1000 milestone (eval 2.265 vs TU09FBO partial's 2.2755 — determinism check; ckpt-1000 2.54 GB on disk; eval ctx1024/batch2 no OOM); `train_target.log`; `runs/target/logs/` |
| 2 | Disk-headroom watch during M3 | `in_progress` | 2026-09-06 probe: 10.13 GB free with ckpt-500+1000 both on disk (~5.1 GB); rotation (limit 3) caps steady state ~7.6 GB → comfortable; report before deleting anything; nothing without user approval | free-disk probes this session; HANDOFF §3b |
...
| 4 | C12 Tier 1 — Evol-Instruct trace SFT of the M3 model | `pending` | User-approved to implement; RUN gated on #1 completion. Launch at gate: `& .\.venv\Scripts\python.exe scripts\sft.py --config configs\sft_t1.yaml --pilot`. Implemented + CPU-validated 2026-09-06 | HANDOFF §8.4; research/c12_distillation_plan.md; commit 5f53a34 |
