...
NONE, mid, pad, seq = 8, int(voc.vocab["|"]), int(voc.vocab["<pad>"]), int(os.environ.get("SEQ", "96"))
...
def composed(ds):
    ...
    for b in range(0, len(ds) - 1, 8):
        ...
        for i, (bm, bx) in enumerate(towers):
            tkv = hs[-1] if os.environ.get("KV_MODE", "perdepth") == "shared" else hs[i + 1]
            bm._strength = 1.0; bm._hold_kv = tkv
            bx._strength = 1.0; bx._hold_kv = tkv
...
(ROOT / "runs/mex" / "gates_dia2c.json").write_text(json.dumps({"gates": gates, "composed": r, "mixed_ce": ce}, indent=2), encoding="utf-8")
