...
towers = []
for i in range(2):
    bm = Bridge(640, 8).cuda(); bm.load_state_dict(mub["bridge" + str(i)]); bm._strength = 0.0; bm._hold_kv = None
    bx = Bridge(640, 8).cuda(); bx.load_state_dict(xb["bridge" + str(i)]); bx._strength = 0.0; bx._hold_kv = None
    towers.append((bm, bx))
...
head = MarkHead(640, 9).cuda(); head.eval()
...
with torch.no_grad():
    for it in [i for i in its if i["task"] == "x2"]:
        ...
        ok2 += int(p == gold)
ok3 = 0; n3 = 0
x3h = torch.nn.Sequential(torch.nn.Linear(640, 640), torch.nn.Tanh(), torch.nn.Linear(640, 2)).cuda()
x3h.eval()
...
x3h.load_state_dict({map3[k]: v for k, v in sd3.items()})
