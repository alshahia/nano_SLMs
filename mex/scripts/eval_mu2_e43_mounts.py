...
E-42 gate (b): widened trunk (runs/mex/mu3_g4/final) + widened E-37a bridges
(runs/mex/mu3_g4/final/bridge_w{0,1}.pt) + widened E-39a head (head_wide.safetensors)
...
for i, br in enumerate(bridges):
    sw = torch.load(str(ROOT / ("runs/mex/mu3_g4/final/bridge_w" + str(i) + ".pt")), weights_only=True)
...
r_on = run(val, "E-43 composed bridges-ON", True)
r_off = run(val, "E-43 trunk-only composed", False)
cmp = (r_on["markpos"] >= r_off["markpos"]) and (r_on["all"] >= r_off["all"])
print(json.dumps({"gate_c_bridge_adds": "PASS" if cmp else "INERT_or_FAIL",
                  "composed": r_on, "trunk_only": r_off,
                  "anchor_E39a": {"all": 0.6533, "markpos": 0.7984}}))
