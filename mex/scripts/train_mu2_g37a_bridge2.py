"""mex/scripts/train_mu2_g37a_bridge2.py — E-37a: bridges on layer 0 AND layer 1, per-depth teacher KV.
...
def main() -> None:
    ...
    ap.add_argument("--dst", default="runs/mex/mu2_g37a")
    ...
    for step in range(1, args.steps + 1):
        ...
        for br in bridges:
            br._strength = 0.0; br._hold_kv = None   # KV pass must NOT ride the bridges
        with torch.no_grad():
            hs = trunk.model(input_ids=clean_ids, output_hidden_states=True).hidden_states
            # bridge_i reads clean layer-i output = hs[i+1]
            for i, br in enumerate(bridges):
                br._hold_kv = hs[i + 1]
        for br in bridges:
            br._strength = strength
    ...
    torch.save({f"bridge{i}": br.state_dict() for i, br in enumerate(bridges)}, str(dst / "bridge.pt"))
    res = dict(phase="mu2_g37a", bridged_mark_acc=acc_b, trunk_only_mark_acc=acc_off,
